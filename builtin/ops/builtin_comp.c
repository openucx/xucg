        ...
        uint64_t offset, void *data, size_t length)
{
    ...
    return ucg_builtin_comp_send_check_frag_many_cb(req, offset, count);
}
        ...
        uint64_t offset, void *data, size_t length)
{
    ...
    return ucg_builtin_comp_send_check_many_cb(req, req->step->fragments, count);
}
        ...
        uint64_t offset, void *data, size_t length)
{
    ...
    return ucg_builtin_comp_send_check_many_cb(req, 0, count);
}
        ...
        ucg_builtin_comp_recv_cb_t *recv_cb, int flags, int nonzero_length)
{
    ...
    int is_batched    = flags & UCG_BUILTIN_OP_STEP_FLAG_BATCHED;
        ...
        if (is_single_msg) {
            if (is_batched) {
                *recv_cb = nonzero_length ?
                        ucg_builtin_comp_reduce_batched_many_then_send_cb :
    ...
    case UCG_PLAN_METHOD_REDUCE_TERMINAL:
         ...
         */
        if (is_batched) {
            *recv_cb = nonzero_length? ucg_builtin_comp_reduce_batched_many_cb:
...
int ucg_builtin_atomic_reduce_full(ucg_builtin_request_t *req,
        uint64_t offset, void *src, void *dst, size_t length)
{
